...
const MODULE_IMPORTS = {
  'ol/layer/Layer': {
    names: ['SourceType'],
    imported: {
      'ol/source/Source~Source': 'SourceType',
    },
    expressions: [`import SourceType from 'ol/source/Source';`],
  },
};
...
/** @type {string[]} */
const EXTERNAL_MODULE_WHITELIST = ['arcgis-rest-api', 'geojson', 'pbf', 'topojson-specification'];
...
/** @type {string[]} */
const GENERIC_TYPE_BLACKLIST = [
  'module:ol/Feature~Feature',
  'module:ol/layer/Layer~Layer',
  'module:ol/layer/BaseVector~BaseVectorLayer',
  'module:ol/source/Vector.VectorSourceEvent',
  'module:ol/source/Vector~VectorSource',
];
...
const IMPORT_PATCHES = {
  ...
  'module:ol/Feature': ['module:ol/geom/Geometry'],
 ...
 * @returns {string}
 */
function getAvailableName(name, names, counter = 1, sep = '_') {
  let availableName = name;
  while (names.indexOf(availableName) != -1) {
    availableName = name + sep + counter;
    counter++;
  }
...
function registerImport(_module, val) {
  ...
  if (val == 'module:ol/format/pbf') {
    if (_imports.imported['pbf']) return _imports.imported['pbf'];
    const alias = getAvailableName('PBF', _imports.names);
    _imports.names.push(alias);
    _imports.imported['pbf'] = alias;
    _imports.expressions.push(`import ${alias} = require('pbf');`);
    return alias;
  }
...
function stringifyType(parsedType, _module, undefinedLiteral = true) {
  ...
  const useGenericType = GENERIC_TYPE_BLACKLIST.indexOf(typeStr) == -1;

  if (useGenericType && typeStr in GENERIC_TYPES) suffix = `<${getGenericType(typeStr)}>`;
  ...
  if (parsedType.type == 'TypeApplication') {
    ...
    switch (typeStr) {
      ...
      default:
        if (useGenericType) typeStr += `<${applications.join(', ')}>`;
        break;
    }
...
const PROCESSORS = {
  ...
  class: (doclet, _module) => {
    ...
    if (doclet.longname in GENERIC_TYPES) name += `<${getGenericType(doclet.longname, false)}>`;
    ...
    if (doclet.augments && doclet.augments.length) {
      ...
      if (augment in GENERIC_TYPES)
        ...
        else augmentName += `<${getGenericType(augment)}>`;
    ...
    const addFire = (eventType, fireType) => {
      ...
      let genericType = getGenericType(fireType);
      if (genericType && genericType == getGenericType(doclet.longname)) genericType = null;
  ...
  method: (doclet, _module, lookupOverrides = true) => {
    ...
    if (doclet.longname in GENERIC_TYPES) name += `<${getGenericType(doclet.longname, doclet.kind != 'function')}>`;
  ...
  typedef: (doclet, _module) => {
    ...
    if (doclet.properties) {
      ...
      let name = doclet.name;
      if (doclet.longname in GENERIC_TYPES) name += `<${getGenericType(doclet.longname)}>`;
 ...
 * @returns {string}
 */
function getGenericType(key, trim = true) {
  if (ANY_GENERIC_TYPES.indexOf(key) != -1 && trim) return 'any';

  let genericType = GENERIC_TYPES[key];
  if (!genericType) return;
  if (trim)
    genericType = genericType
      .split(',')
      .map(t => {
        t = t.replace(/^(\w+)\s?(extends|=)\s?\w+$/, '$1').trim();
        return t;
      })
      .join(', ');
...
function extractGenericTypes(initial = true, strict = false) {
  if (initial)
    find({ tags: { isArray: true } }).forEach(doclet => {
      if (GENERIC_TYPE_BLACKLIST.indexOf(doclet.longname) != -1) return;
